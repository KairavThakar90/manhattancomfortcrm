...
export const CRMProvider = ({ children }: { children: ReactNode }) => {
    ...
    const handleAddAudit = (poId: string, action: string, prev: string, next: string, browser?: string, ip?: string) => {
        const newAudit: AuditLog = {
            id: 'AUD-' + Math.floor(100 + Math.random() * 900),
            timestamp: new Date().toISOString().slice(0, 19).replace('T', ' '),
            user: 'You (' + userRole + ')',
    ...
    const handleTriggerSync = () => {
        ...
        const updatedPOs = purchaseOrders.map((p) => {
            if (p.id === 'PO-10027') {
                return { ...p, productionStage: 'Assembly', commentsCount: (p.commentsCount || 0) + 1 };
            }
            return p;
        });
        handleUpdatePOs(updatedPOs);
    ...
    return (
        <CRMContext.Provider value={{
            ...
            purchaseOrders, setPurchaseOrders, handleUpdatePOs,
            vendors, setVendors, handleUpdateVendors,
            emailLogs, setEmailLogs, handleAddEmailLog,
            comments, setComments, handleAddComment,
            chats, setChats, handleAddChatMessage,
            notifications, setNotifications, handleNotificationClick, handleMarkAllNotificationsRead,
            activityLogs, setActivityLogs, handleAddActivity,
            auditLogs, setAuditLogs, handleAddAudit,
            syncLogs, setSyncLogs, handleTriggerSync,
