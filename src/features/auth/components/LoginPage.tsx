...
function LoginPageContent({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (username && password && !loading) {
      try {
        ...
        // Error is expected to be handled primarily by parent feeding `error` prop.
        // We catch to prevent step from advancing.
      }
    }
  };
  ...
  const handleOtpSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onVerifyOtp(username, otp, rememberMe);
    } catch (err) {
      // Let parent handle UI errors
    }
  };
          ...
          {step === 'login' ? (
            ...
            >
              ...
              {/* <Tooltip
                ...
                place="bottom"
                className="z-50 text-xs font-bold shadow-lg"
              /> */}
            </form>
          ) : (
            <form
